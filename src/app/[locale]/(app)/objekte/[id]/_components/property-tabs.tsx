...
export function PropertyTabs({ property: p, analysis: a, documents, isPro, locale }: PropertyTabsProps) {
  ...
  async function triggerAnalysis() {
    ...
    try {
      ...
      if (!res.ok) {
        setTriggerMsg(data.error ?? `Analyse-Fehler ${res.status}`);
      } else {
        // Analyse ist fertig - Seite mit neuen DB-Daten neu laden
        router.refresh();
      }
    } catch {
            ...
            <div className="rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
              <div className="flex flex-col items-center gap-4 py-8 text-center">
                ...
                <button
                  ...
                  className="flex items-center gap-2 rounded-full border border-zinc-300 px-4 py-2 text-sm text-zinc-600 hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-400"
                >
              ...
              <div className="mb-4 flex items-center justify-between">
                <h3 className="text-base font-semibold text-zinc-900 dark:text-zinc-50">
                  {a.analysis_model === "algorithmic-fallback"
                    ...
                    : a.analysis_model === "max-zvg-portal"
                      ? "KI-Analyse (ZVG-Portal-Daten)"
                      : "KI-Risikoanalyse"}
                </h3>
